...
export const CVModal: React.FC<CVModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md" onClick={onClose}>
            <div 
                className="dark:bg-slate-900/80 bg-white/95 backdrop-blur-xl border dark:border-white/10 border-slate-200 rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-8 relative shadow-[0_0_50px_rgba(0,0,0,0.5)] animate-[fade-in-up_0.3s_ease-out]" 
                onClick={e => e.stopPropagation()}
            >
                <button 
                    onClick={onClose} 
                    className="absolute top-4 right-4 p-2 rounded-full dark:bg-white/5 bg-slate-100 text-slate-400 hover:text-white dark:hover:text-white hover:bg-red-500/20 hover:border-red-500/50 border border-transparent transition-all"
                >
                    <X size={24} />
                </button>
                
                <h2 className="text-3xl font-bold text-center mb-10 text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-pink-500">Sobre as Autoras</h2>

                <div className="grid md:grid-cols-2 gap-10">
                    {/* Person 1 */}
                    <div className="space-y-6 dark:bg-white/5 bg-slate-50 p-6 rounded-2xl border dark:border-white/5 border-slate-100">
                        <div className="flex items-center gap-5 border-b dark:border-white/10 border-slate-200 pb-6">
                            <img src="https://i.imgur.com/lG4TKTk.png" alt="Albeísa" className="w-24 h-24 rounded-full border-2 border-orange-500 p-1 object-cover" />
                            <div>
                                <h3 className="text-xl font-bold dark:text-white text-slate-800">Albeísa Cleyse</h3>
                    ...
                    <div className="space-y-6 dark:bg-white/5 bg-slate-50 p-6 rounded-2xl border dark:border-white/5 border-slate-100">
                         <div className="flex items-center gap-5 border-b dark:border-white/10 border-slate-200 pb-6">
                            <img src="https://i.imgur.com/ujSR6FD.png" alt="Terezinha" className="w-24 h-24 rounded-full border-2 border-orange-500 p-1 object-cover" />
                            <div>
                                <h3 className="text-xl font-bold dark:text-white text-slate-800">Terezinha de Jesus</h3>
...
export const ImageModal: React.FC<ImageModalProps> = ({ src, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl p-4" onClick={onClose}>
            <button 
                onClick={onClose} 
                className="absolute top-6 right-6 text-white/50 hover:text-white bg-white/10 hover:bg-orange-500/80 p-3 rounded-full transition-all z-50"
            >
                <X size={32} />
            </button>
            <img 
                src={src} 
                alt="Full size" 
                className="max-w-full max-h-[90vh] rounded-xl shadow-2xl border border-white/10 animate-[scale-in_0.3s_cubic-bezier(0.16,1,0.3,1)]"
                onClick={e => e.stopPropagation()}
            />
        </div>
    );
